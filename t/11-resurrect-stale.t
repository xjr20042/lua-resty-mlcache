...
=== TEST 4: get() accepts 'opts.resurrect_ttl' option to override instance option
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
...
=== TEST 5: get() resurrects a nil stale value (negative cache)
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
...
=== TEST 6: get() resurrects a nil stale value (negative cache) in 'opts.shm_miss'
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
...
=== TEST 7: get() ignores cb return values (except err) upon stale resurrection
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
...
=== TEST 8: get() does not resurrect a stale value when callback throws error
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
...
=== TEST 9: get() returns error and data on lock timeout but does not resurrect
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
...
=== TEST 10: get() only returns nil and error on lock timeout if resurrect is not desired
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
...
=== TEST 11: get() does not resurrect a stale value if no 'resurrect_ttl' is set on the instance
--- http_config eval: $::HttpConfig
--- config
    location = /t {
        content_by_lua_block {
